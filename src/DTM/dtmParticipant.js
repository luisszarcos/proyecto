...
module.exports = {
    ...
    Participant: class Participant {
        constructor (coordinatorDir, host, pendingLogs) {
        ...
        attachCommunicationEvents (socket) {
            ...
            socket.on('lockListGot', function ( data ) {
                ...
                /*
                parentScope.pendingLogs.forEach ( element => {
                ...
                fs.readdirSync('./logs/pending/').forEach( file => {
                    ...
                    // append to array
                    pendingLogs2.push(JSON.parse(contents));
                });
                */
                //parentScope.pendingLogs.pendingLogs
                // traverse the array and check for matches:
                console.log(parentScope.pendingLogs.pendingLogs);
                for (var i=0; i<parentScope.pendingLogs.pendingLogs.length; i++) {
                    // iterate over the lock requested:
                    for (var j=0; j<data.length; j++) {
                        if ( parentScope.pendingLogs.pendingLogs[i].tablesLocked.includes(data[j]) ) {
                ...
                .then((value) => {
                    ...
                    if (value[0][0] == undefined ) {
                        ...
                        parentScope.logInfo.operation == "20")
                        {
                            ...
                            */
                            console.log("*****DATA::::::");
                            console.log(parentScope.coordinatorDir);
                            console.log(parentScope.participantInfo.host);
                            console.log(parentScope.logInfo.parameterList);

                            var tempHost = parentScope.coordinatorDir.substring(parentScope.coordinatorDir, parentScope.coordinatorDir.length - 5);
                            console.log("TEMPHOST:");
                            console.log(tempHost);
                            if (tempHost == parentScope.participantInfo.host) {
                                parentScope.logInfo.parameterList[3] = true;
                            } else {
                                parentScope.logInfo.parameterList[3] = false;
                            }
            ...
            socket.on('disconnect', function ( data ) {
                ...
                if (parentScope.hasReceivedSecondConfirm == 1) {
                    ...
                    .then((value) => {
                        ...
                        parentScope.pendingLogs.pendingLogs.push(
                            parentScope.logInfo
                        );
                        // release locks:
